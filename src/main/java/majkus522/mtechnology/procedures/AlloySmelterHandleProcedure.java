...
import majkus522.mtechnology.AlloySmelterData;

import java.util.List;

public class AlloySmelterHandleProcedure
{
	public static void execute(LevelAccessor world, double x, double y, double z)
	{
		BlockPos position = new BlockPos(x, y, z);
		List<Recipe> recipes = AlloySmelterData.getRecipes();
		MachineModifire modifire = AlloySmelterData.getModifire(ExtensionMethods.getCountFromBlock(world, position, 3), ExtensionMethods.getCountFromBlock(world, position, 4));
			...
			else if(ExtensionMethods.getNumberNbt(world, position, "timeLeft") <= 0)
			{
				for(Recipe recipe : recipes)
				{
					if(recipe.name == ExtensionMethods.getStringNbt(world, position, "input"))
					{
						...
						else if(recipe.output[0].getCount() + ExtensionMethods.getCountFromBlock(world, position, 2) <= 64 &&
		...
		else
		{
			for(Recipe recipe : recipes)
			{
				...
				else if(recipe.input[0].getItem() == ExtensionMethods.getItemFromBlock(world, position, 1).getItem() &&
						recipe.input[0].getCount() <= ExtensionMethods.getItemFromBlock(world, position, 1).getCount() &&
						recipe.input[1].getItem() == ExtensionMethods.getItemFromBlock(world, position, 0).getItem() &&
						recipe.input[1].getCount() <= ExtensionMethods.getItemFromBlock(world, position, 0).getCount())
				{
					ExtensionMethods.setLogicNbt(world, position, "processing", true);
					ExtensionMethods.setStringNbt(world, position, "input", recipe.name);
					ExtensionMethods.setNumberNbt(world, position, "timeLeft", recipe.time * modifire.speed);
