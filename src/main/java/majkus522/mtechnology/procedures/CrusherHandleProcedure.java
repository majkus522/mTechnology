...
import majkus522.mtechnology.CrusherData;

import java.util.List;

public class CrusherHandleProcedure 
{
	public static void execute(LevelAccessor world, double x, double y, double z) 
	{
		BlockPos position = new BlockPos(x, y, z);
		List<Recipe> recipes = CrusherData.getRecipes();
		MachineModifire modifire = CrusherData.getModifire(ExtensionMethods.getCountFromBlock(world, position, 2), ExtensionMethods.getCountFromBlock(world, position, 3));
		if(ExtensionMethods.getLogicNbt(world, position, "processing"))
		{
			...
			else if(ExtensionMethods.getNumberNbt(world, position, "timeLeft") <= 0)
			{
				for(Recipe recipe : recipes)
				{
					...
					{
						if(ExtensionMethods.getCountFromBlock(world, position, 1) == 0)
						{
							ExtensionMethods.setItemForBlock(world, position, 1, recipe.output[0]);
							ExtensionMethods.setLogicNbt(world, position, "processing", false);
						}
						else if(recipe.output[0].getCount() + ExtensionMethods.getCountFromBlock(world, position, 1) <= 64 &&
						   recipe.output[0].getItem() == ExtensionMethods.getItemFromBlock(world, position, 1).getItem())
						{
							ExtensionMethods.setItemForBlock(world, position, 1, recipe.output[0], recipe.output[0].getCount() + ExtensionMethods.getCountFromBlock(world, position, 1));
							ExtensionMethods.setLogicNbt(world, position, "processing", false);
						}
						else
							return;
